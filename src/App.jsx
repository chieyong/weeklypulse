...
export default function App(){
  ...
  return (<div onClick={()=>{if(colorPick)setColorPick(null);}} style={{background:"#FDF6EE",minHeight:"100vh",width:"100%",boxSizing:"border-box",fontFamily:"Nunito,sans-serif",color:"#3D2E1F"}}>
    ...
    <div className="wk-outer" style={{width:"100%",minHeight:"100vh",display:"flex",flexDirection:"column",padding:"20px 24px 52px",position:"relative"}}>
      <div style={{position:"fixed",top:12,right:12,zIndex:220,display:"flex",alignItems:"center"}} className="wk-edit-btn">
        <button onClick={()=>setTabEdit(!tabEdit)} style={{width:22,height:22,borderRadius:"50%",border:"none",background:"transparent",color:tabEdit?"#C75D3A":"#8A7560",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",transition:"color .2s",boxShadow:"none",fontSize:"12px",lineHeight:1,fontFamily:"Nunito",padding:0}} title="Edit personas & weeks" aria-label="Edit personas & weeks">
      ...
      {isMobile&&tabEdit&&(
        <div style={{margin:"2px auto 10px",width:"100%",maxWidth:460,display:"flex",flexDirection:"column",gap:6,padding:"0 4px"}}>
          <div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}>
            <span style={{fontSize:".58rem",color:"#8A7560",fontWeight:700,textTransform:"uppercase",letterSpacing:".04em"}}>Personas</span>
            {order.map(id=>{const mm=members[id];if(!mm)return null;return (<div key={id} style={{display:"inline-flex",alignItems:"center",gap:4,padding:"2px 8px",borderRadius:12,border:"1px solid rgba(61,46,31,.14)",background:"white",fontSize:".62rem",color:"#3D2E1F"}}><button onClick={()=>{setMember(id);setMultiMode("enkel");setViewSnap(null);}} style={{border:"none",background:"transparent",padding:0,cursor:"pointer",fontFamily:"Nunito",fontSize:".62rem",color:"#3D2E1F"}}>{mm.emoji} {mm.label}</button>{order.length>1&&<button onClick={()=>removeMember(id)} style={{width:14,height:14,borderRadius:"50%",border:"1px solid #E0D4C6",background:"#fff",color:"#C75D3A",fontSize:".5rem",lineHeight:1,cursor:"pointer",padding:0}}>x</button>}</div>);})}
            <button onClick={()=>setShowAdd(true)} style={{width:18,height:18,borderRadius:"50%",border:"1.5px dashed rgba(61,46,31,.2)",background:"transparent",color:"#8A7560",fontSize:".65rem",cursor:"pointer",padding:0,lineHeight:1}}>+</button>
          </div>
          <div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}>
            <span style={{fontSize:".58rem",color:"#8A7560",fontWeight:700,textTransform:"uppercase",letterSpacing:".04em"}}>Weeks</span>
            {memberSnaps.length===0&&<span style={{fontSize:".6rem",color:"#B8A08A"}}>No saved weeks yet</span>}
            {memberSnaps.map(snap=>(<div key={snap.id} style={{display:"inline-flex",alignItems:"center",gap:4,padding:"2px 8px",borderRadius:12,border:"1px solid rgba(61,46,31,.14)",background:"white",fontSize:".62rem",color:"#3D2E1F"}}><button onClick={()=>{setViewSnap(snap.id);setViewSnapMonday(snap.monday);setMultiMode("enkel");}} style={{border:"none",background:"transparent",padding:0,cursor:"pointer",fontFamily:"Nunito",fontSize:".62rem",color:"#3D2E1F"}}>{snap.label}</button><button onClick={()=>deleteSnapshot(snap.id)} style={{width:14,height:14,borderRadius:"50%",border:"1px solid #E0D4C6",background:"#fff",color:"#C75D3A",fontSize:".5rem",lineHeight:1,cursor:"pointer",padding:0}}>x</button></div>))}
          </div>
        </div>
      )}
      {/* Bio */}
